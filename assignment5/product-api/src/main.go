...
func main() {
	store := NewStore() // Create data store

	r := gin.New()          // Create Gin server
	r.Use(gin.Logger())     // Log all requests
	r.Use(gin.Recovery())   // Recover from crashes
	r.Use(authMiddleware()) // Check auth on every request
